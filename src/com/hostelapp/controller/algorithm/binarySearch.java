...
public class binarySearch {
  ...
  public hostelmodel searchByName(String searchValue, List<hostelmodel> bookings,
            int left, int right) {
        ...
        // checks whether searchKey lies on mid point
        if (searchValue.compareToIgnoreCase(bookings.get(mid).getName()) == 0) {
            return bookings.get(mid);
        } else if (searchValue.compareToIgnoreCase(bookings.get(mid).getName()) < 0) {
            return searchByName(searchValue, bookings, left, mid - 1);
        } else {
            return searchByName(searchValue, bookings, mid + 1, right);
        }
  }
}
